...
class NavigationState(Enum):
    ...
    Attributes:
        ...
        AWAITING_RETURN: Passive state waiting for the user confirmation to guide them to the object.
...
class NavManager(Node):
    ...
    Atributes:
        ...
        vision_pub (Publisher): Sends commands ("start_vis", "stop_vis") to the ticare_vision package to activate or deactivate the camera during the search phase.
        communication_pub (Publisher): Communicates mission status ("home", "object_point") to the ticare_communication package.

        vision_sub (Subscription): Receives object detection status ("object_detected") from ticare_vision package.
        communication_sub (Subscription): Receives mission start commands ("start_nav", "return") from ticare_communication package.
    ...
    def __init__(self) -> None:
        """Initializes the NavManager node, sets up publishers, subscribers, services and action clients."""
        ...
        self.communication_sub = self.create_subscription(String, 'com2nav', self.communication_callback, 10)
